...
p+geom_boxplot()+theme_bw()+labs(x="Plate Location", y="Species Count/Plate")

p2<-ggplot(divdata,aes(PlateTrt,count))
p2+geom_boxplot()+theme_bw()+labs(x="Plate Treatment", y="Species Count/Plate")
p3<-ggplot(data=divdata2,aes(x=Dist_m,y=count))+geom_boxplot(aes(group=Dist_m))
p3+facet_grid(PlateTrt~Status,scales="free",as.table=FALSE)+labs(y="Species Count/Plate",x="Distance from Marina")+theme_bw()+theme(axis.text=element_text(size=10),axis.title=element_text(size=10))+theme(strip.text.x=element_text(size=10),strip.text.y=element_text(size=10))
